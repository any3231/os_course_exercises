...
答：特权指令只能在ring 0使用。一条指令在不同特权级下能访问的数据范围是不一样的；一条指令在不同特权级下的行为是不一样的。
...
1）访问数据时，需要检查当前特权级是否等于或高于要访问的数据段的DPL。 

如果是通过段寄存器SS访问数据段，则要求CPL、RPL = DPL。 

2）如果代码涉及到控制转移，则一般只允许低特权级调用高特权级代码。 

3）发生中断或异常时，需要判断当前的门描述符的特权级和目标代码的特权级。
...
压栈内容不同，多了一个SS：ESP的压栈；进行了栈切换。

4. 如何利用int和iret指令完成不同特权级的切换？

按照需要构造栈，通过int和iret指令进行切换。
